...
int dustPin = A1;
float dustVal = 0;
float dustDensity ; // 미세먼지 변수 (mg/m3)
float dustDensityug; // 미세먼지 변수 [ug/m3]
float voMeasured = 0;
float calcVoltage = 0;
...
String x="37.88662080000002",y="127.73982250000002",tempx,tempy; // 한림대학교 
...
SoftwareSerial esp8266(12,11); // RX/TX 설정, serial 객체생성
//=================co2 sansor =-==============
...
#define ZERO_POINT_VOLTAGE (0.06) //define the output of the sensor in volts when the concentration of CO2 is 400PPM
#define REACTION_VOLTGAE (0.03) //define the voltage drop of the sensor when move the sensor from air into 1000ppm CO2
/*****************************Globals***********************************************/
...
Serial.print("MG-811 Demostration\n");
  ...
  connectWifi();
  //==============wifi end===================
 MsTimer2::set(10000,gpsLocation); // 10000ms period
  MsTimer2::start();
}
//============loop 문====================
void loop(){ // main loop !!
...
//=================co2 sansor =-==============
//==================wifi modul=================
String ssid ="Dr.Will"; //wifi 의 이름 
String password="00941129"; // wifi의 pw 
void connectWifi() {
   String cmd = "AT+CWJAP=\"" +ssid+"\",\"" + password + "\""; //  Connect 요청
 digitalWrite(LED_BUILTIN, LOW);
  Serial.println(cmd);
 for(int i=0;i<3;i++){
    esp8266.println(cmd);
...
/*
  ...
  */
  ...
  cmd += "jeane1993.cafe24.com"; // 서버 접속 IP or 주소
  cmd += "\",80";           // 서버 접속 포트, 80
  esp8266.println(cmd);
  if(esp8266.find("Error")){
    ...
    // prepare GET string
  String getStr = "GET /insertdata?";
  getStr +="x="; 
  getStr += String(x); // x
  getStr +="&y=";
  getStr += String(y); // y 
  getStr +="&element1=";
  getStr += String(abs(dustDensityug));// 이산화탄소 
  getStr +="&element2=";
  getStr += String(percentage); // 미세먼지 
  getStr +="&element3=";
  getStr += String(co); // 일산화탄소
  /*
  getStr +="&field3=";
  getStr += String(temperature); // 온도*/ 
  getStr += " HTTP/1.1\r\nHost: jeane1993.cafe24.com\r\n\r\n";
  // send data length
  cmd = "AT+CIPSEND=";
